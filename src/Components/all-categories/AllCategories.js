...
const AllCategories = () => {
    const { allCategories, loader, error } = useAppSelector(
    ...
    useEffect(() => {
        if (!window.localStorage.getItem('allCategories')) {
            ...
            getAllCategories(
                JSON.parse(window.localStorage.getItem('allCategories'))
            );
        }
    }, []);
            ...
            {allCategories && !loader && !error ? (
                <>
                    ...
                    {allCategories.map((el, i) => (
                        <View key={+el.id} index={i} {...el} id={+el.id} />
                    ))}
                </>
            ) : (
                <div className='not-found'>DONT HAVE ANY MEALS CATEGORY</div>
            )}
        </div>
    );
};
